...
import MuseumArea from './MuseumArea';
...
export default class Town {
   ...
   */
  public async initializeFromMap(map: ITiledMap) {
    console.log('here');
    ...
    const museumAreas = await Promise.all(
      objectLayer.objects
        .filter(eachObject => eachObject.type === 'MuseumArea')
        .map(eachViewingAreaObject =>
          MuseumArea.fromMapObject(eachViewingAreaObject, this._broadcastEmitter),
        ),
    );
